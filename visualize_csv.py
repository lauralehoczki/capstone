...
def generate_graph():
    # Get all the CSV files for each unique part type
    # It doesn't matter which module we're looking at first, because we're only going to graph
    # the ones that have matching part type output for the other two modules
    part_type_list = []
    for i in os.listdir(DATA_PATH):
        if os.path.isfile(os.path.join(path, i)) and "M1998_052_" in i:
            part_type_list.append(i)
    
    # Check if the other two modules have the matching part type output
    # If they do, open the three matching csv's and read in the data
    for part_type in part_type_list:
        m052_path = part_type
        m04_path = DATA_PATH + "M2002_04_" + part_type + ".csv"
        m06_path = DATA_PATH + "M1998_06_" + part_type + ".csv"
        if os.path.isfile(os.path.join(path, m04_path)) and os.path.isfile(os.path.join(path, m06_path)):
